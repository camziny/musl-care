...
export function SimpleUploadButton({ inputId }: SimpleUploadButtonProps) {
  ...
  const { startUpload, isUploading } = useUploadThing("imageUploader", {
    ...
    onClientUploadComplete(result) {
      ...
      if (result && result.length > 0) {
        ...
        const input = document.querySelector(`input[name="${inputId}"]`) as HTMLInputElement;
  ...
  return (
    <div className="flex flex-col items-center space-y-4">
      <label htmlFor="upload-button" className="cursor-pointer flex flex-col items-center">
        <UploadSvg />
        {isUploading && (
          <div className="mt-2">
            <LoadingSpinnerSVG />
      ...
      <input id="upload-button" type="file" className="sr-only" onChange={handleFileChange} multiple={false} accept="image/*" />
    </div>
  );
}
...
function LoadingSpinnerSVG() {
  return (
    <svg width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="black" className="spinner">
      <style>{`@keyframes spinner{0%{transform:rotate(0deg);}100%{transform:rotate(360deg);}} .spinner{animation:spinner 1s linear infinite;}`}</style>
